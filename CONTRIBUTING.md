...
1. **Declare the field** in `pkg/schema/dsconfig.json` under `fields`, and add its `id` to
   ...
   convention, e.g. `jsonData_clusterIdentifier`.
2. **Add the matching Go field** to `RedshiftDataSourceSettings` in `pkg/redshift/models/settings.go` with a json tag equal
...
- `JSONDataMatchesStruct` / `JSONDataTypesMatchStruct` — the schema and `RedshiftDataSourceSettings` disagree on keys or types. Update whichever side is behind.
